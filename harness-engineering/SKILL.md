...
## 第十八章：工具路由矩阵⭐ 第十五轮学习新增

**根据任务自动选择工具组合：**

| 路由策略 | 机制 | 适用场景 |
|---------|------|---------|
| 关键词路由 | 正则匹配任务关键词 | 简单任务 |
| 嵌入路由 | 任务描述向量化匹配 | 复杂/模糊任务 |
| Few-shot路由 | 检索历史成功案例复用 | 已有执行历史 |
...
TOOL_ROUTES = {
    ...
    "code_modification": {
        "triggers": ["修改", "重构", "修复"],
        "tools": ["read_file", "search_files", "patch", "write_file"]
    }
}
```

## 第十九章：三层结果缓存⭐ 第十五轮学习新增
