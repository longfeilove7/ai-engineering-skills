...
## 九、跨模型Prompt自适应⭐ 第十六轮学习新增

> 来源：GPT-4/Claude/Gemini官方指南

### 9.1 三模型Prompt关键差异

| 模型 | 最强特性 | 最佳格式 | 核心弱点 |
|------|---------|---------|---------|
| GPT-4 | JSON mode原生支持 | JSON + System Message | 长prompt后半注意力下降 |
| Claude | XML标签解析、指令遵循最严格 | XML结构化 | 过度谨慎 |
| Gemini | 超长上下文(1M+)、多模态原生 | JSON + system_instruction | 指令精确度不如Claude |

### 9.2 跨模型通用性四大原则

1. **结构化优先**：Markdown/XML > 自然语言段落
2. **显式优于隐式**：具体指令 > 模糊描述
3. **格式锚定**：所有模型都用统一的输出结构模板
4. **Few-shot一致性**：3个高质量示例 > 10个中等示例

### 9.3 模型适配器模式

```
通用Core（任务/约束/输出格式）→ 模型适配器层（格式优化/特性利用）
```
...
## 十一、上下文感知动态Prompt生成⭐ 第十九轮研究新增

> 来源：对话状态追踪(DST)、情绪检测、任务阶段管理、LangGraph/DSPy
> 完整研究报告：references/context-aware-dynamic-prompt-research.md

### 11.1 核心理念

传统prompt是常量，动态prompt是**对话状态的函数**：

```
静态:  prompt = 固定模板
动态:  prompt = f(对话历史, 用户情绪, 任务进展, 环境状态)
```
...
| 层 | 内容 | 变更频率 | 示例 |
|----|------|---------|------|
| **不变层** | 安全规则、核心角色、输出格式 | 几乎不变 | "你是X角色，不做Y" |
| **半稳定层** | 领域知识、风格偏好、任务约束 | 话题/阶段切换时 | "用表格格式，专业技术语言" |
| **动态层** | 当前焦点、最近上下文、情绪微调 | 每轮或每几轮 | "用户刚提到X，当前关注Y" |

放置策略：不变层开头（注意力最高）→ 半稳定层中间 → 动态层结尾（最近效应）
...
|---------|------|------|
| <5轮 | 全部注入 | 无损 |
| 5-15轮 | 最近3轮 + 关键信息提取 | 平衡 |
| >15轮 | 最近3轮 + 压缩摘要 | 防超限 |

**上下文压缩**：用LLM将早期历史压缩为500token摘要，保留关键实体、决策、偏好。

**动态Few-Shot**：根据当前话题从示例库中embedding检索最相关示例（MMR策略：0.7相似度+0.3多样性）。
...
| 中性 | "主动提供额外有用信息" |
| 轻度不满 | "先确认理解是否正确，再给方案" |
| 高度不满 | "简短承认问题，直接给方案，不解释辩护" |
| 困惑 | "分步骤说明，用更简单的语言" |

**关键原则**：看趋势不看单条、不显式提情绪、不讨好、考虑文化差异。

### 11.5 任务阶段约束更新

| 阶段 | 信号 | 注入约束 |
|------|------|---------|
| 探索 | 前2轮、需求模糊 | "主动提问，不急于给方案" |
| 规划 | 用户确认需求 | "先列大纲，确认后展开" |
| 执行 | 用户批准计划 | "专注当前部分，保持格式一致" |
| 审查 | 产出初稿 | "检查一致性、数据准确性" |
| 定稿 | 用户确认内容 | "输出最终版本，标注修改点" |

**约束管理**：安全约束永不移除；阶段约束按阶段切换；用户约束优先级最高。
...
**版本管理**：每次演化记录 `{trigger, diff, turn}` 以支持回滚。
...
短对话（<5轮）：静态 + 最近3轮
中对话（5-15轮）：+ 情绪检测 + 阶段约束
长对话（>15轮）：+ 上下文压缩 + 动态Few-Shot
```

### 11.7 Pitfalls

1. **不要每轮大改prompt**：渐进演化，频繁大改让模型"迷失"
2. **不要过度注入历史**：太多上下文稀释当前指令注意力（Lost in the Middle）
3. **不要过度解读单条情绪**：看趋势，用滑动窗口
4. **不要显式提情绪**："我注意到您不满"可能适得其反
5. **阶段切换要有滞后**：避免在两个阶段间震荡
6. **安全约束永不移除**：不管什么阶段、什么情绪
7. **用户可覆盖一切**：说"跳过X"时尊重用户
8. **成本意识**：每轮重建prompt有token成本，短对话不需要动态生成
