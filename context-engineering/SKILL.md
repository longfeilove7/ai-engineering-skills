...
## 二十一、多模态上下文管理⭐ 第十二轮学习新增

> 来源：GPT-4V/LLaVA/ColPali/LMQL + Hermes实践总结

**核心问题：图像+代码+文档混合时，如何高效利用token预算？**

### 21.1 视觉Token成本

| 模型 | 1张图token消耗 | 说明 |
|------|---------------|------|
| GPT-4o低分辨率 | ~85 tokens | 快速模式 |
| GPT-4o高分辨率 | ~765 tokens | tile机制 |
| Claude 3.5 | ~1,600 tokens | 固定消耗 |
| Gemini 1.5 | ~258 tokens | 原生高效 |

**规则：1张图 ≈ 500-2000 tokens，等同1-4页文本。**

### 21.2 图像降级策略（Progressive Image Disclosure）
...
| L1 缩略 | 低分辨率 | ~85 | 快速预览 |
| L2 标准 | 中分辨率 | ~300 | 常规分析 |
| L3 高清 | 原始分辨率 | ~1000+ | 需读文字/细节 |

**最佳实践**：图像信息先文字化，只在需要"看到"时才保留原图。

### 21.3 代码上下文分层

| 层 | 内容 | token/100行 | 适用场景 |
|---|------|-----------|---------|
| L0 签名 | 函数签名+类定义+类型 | ~100 | 理解结构 |
| L1 摘要 | 签名+关键逻辑注释 | ~300 | 理解行为 |
| L2 片段 | 相关函数完整实现 | ~500 | 修改代码 |
| L3 全文 | 完整文件 | ~2000 | 深度分析 |

**代码+文档混合优先级**：P0当前代码+错误 → P1被调用函数 → P2测试/文档 → P3依赖库

### 21.4 动态Token预算分配

```
总预算B = 128K，固定分配：系统指令5K + 输出预留16K = 21K

文本分析任务：文档50K(47%) + 图像5K(5%) + 历史30K(28%) + 工具12K(11%)
代码开发任务：代码40K(37%) + 相关20K(19%) + 需求15K(14%) + 历史15K(14%)
视觉分析任务：图像30K(28%) + 描述10K(9%) + 指令15K(14%) + 历史25K(23%)
混合方案任务：文档30K(28%) + 参考15K(14%) + 图表10K(9%) + 草稿20K(19%)
```

**裁剪顺序（最先→最后）**：参考资料 → 代码注释/空行 → 图像降级 → 旧对话 → 工具输出摘要 → 相关文档 → 最近对话 → 当前任务核心

### 21.5 多模态RAG融合策略

**晚融合（推荐）**：各模态独立检索 → 统一相关性分数 → 上下文感知补充

```
文本检索: BM25 + embedding混合
图像检索: CLIP向量 或 图像→caption→文本检索
代码检索: AST结构 + 语义检索
融合: 检查文本引用→补充图像；图像含代码→补充代码文件
```
...
```python
# 图像文字化后存储（省95% token）
memory_save(content="架构图：前端Vue→Gateway→用户/订单/支付服务→MySQL+Redis", type="fact")

# 代码签名检索（省70% token）
search_files(pattern="def authenticate", target="content", file_glob="*.py")

# 图像按需加载
vision_analyze(image_url="screenshot.png", question="这里有什么错误？")

# ColPali方式：文档页面直接视觉检索，跳过OCR
```

### 21.7 核心原则

1. **图像即成本** — 默认用文字替代，需要"看到"才用图
2. **代码分层** — 签名→摘要→片段→全文，按需加载
3. **晚融合优先** — 各模态独立检索，组装时融合
4. **降级兜底** — 图→文字，代码→签名，始终有降级方案
5. **Lost in the Middle** — 重要多模态信息放首尾

## 二十二、上下文的个性化适配⭐ 第二十二轮深度研究

> 来源：Mem0用户偏好层 + 角色感知上下文 + 多语言token经济学

**核心问题：同一个问题，不同用户需要完全不同的上下文。个性化适配不是锦上添花，是效率核心。**

### 22.1 根据用户角色调整上下文策略

| 角色 | 优先上下文 | 淘汰上下文 | 输出格式 |
|------|-----------|-----------|---------|
| 开发者 | 代码/API/错误栈 | 业务背景 | 代码块、diff |
| 售前 | 客户需求/方案/报价 | 底层代码 | 方案文档、表格 |
| PM | 进度/风险/里程碑 | 技术细节 | 摘要、甘特图 |
| 运维 | 配置/日志/runbook | 需求文档 | 命令、拓扑图 |

**动态角色识别**：从用词模式、问题类型、工具调用行为推断角色，置信度>0.8时自动应用角色策略。

### 22.2 用户历史偏好优化

三层学习体系：
- **L1 显式偏好**：用户直接说 → 立即应用
- **L2 隐式偏好**：行为统计推断 → 置信度达标后应用
- **L3 任务偏好**：历史任务模式 → 新任务自动预加载

偏好衰减：最近7天权重1.0 → 30天0.7 → 90天0.4 → 90天+0.2

上下文选择算法：`score = 0.5*语义相关性 + 0.3*偏好匹配 + 0.2*时间衰减`

### 22.3 专业领域差异化管理

| 领域 | 压缩策略 | RAG策略 | 知识注入优先级 |
|------|---------|---------|---------------|
| 软件开发 | 函数级/代码感知 | AST+语义 | 代码>API>架构>需求 |
| 政府采购 | 条款级/法规感知 | 关键词+法规号 | 法规>模板>案例>方案 |
| 医疗 | 段落级/术语感知 | 语义+MeSH | 指南>药品>病历>文献 |
| 金融 | 指标级/时间感知 | 时间序列+关键词 | 行情>合规>模型>报告 |

术语感知压缩：专家用户可更激进压缩（省略已知上下文），非技术用户需展开解释。

### 22.4 多语言上下文处理

**中文token成本是英文的7-8倍**，这是中文用户上下文管理的核心挑战。

| 策略 | 说明 |
|------|------|
| 语言感知压缩 | 中文压缩阈值降低10%（50%触发，而非60%） |
| 混合语言 | 技术术语保留英文原文，概念解释用用户语言 |
| 双语索引 | 同一概念同时索引中英文关键词 |
| 输出适配 | 跟随用户语言，代码注释跟随用户语言 |

中文特殊优化：分词感知搜索、同义词扩展、标点归一化、数字格式统一。

### 22.5 个性化适配流水线

```
用户输入 → 角色识别 → 领域识别 → 语言检测 → 上下文组装(个性化权重) → 输出适配
```
